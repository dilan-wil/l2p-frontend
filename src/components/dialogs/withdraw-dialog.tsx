...
import { Loader2, Smartphone, ArrowDownCircle } from "lucide-react";
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const res = await axios.post(
        `${baseUrl}/accounts/withdraw?accountId=${accountId}&amount=${amount}&phoneNumber=${phoneNumber}&method=${method}`,
        {},
        {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await axios.get(
          `${baseUrl}/accounts/check-payout-status?transactionId=${transactionId}`,
          {
            headers: { Authorization: `Bearer ${accessToken}` },
          }
        );
        ...
        console.error("Check payout status failed", err);
      }
    }, 5000);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 cursor-pointer hover:opacity-90 gap-2">
        ...
        <DialogHeader>
          <DialogTitle>
            {waiting
              ...
              : account
              ? `Withdraw from ${account.type} - ${maskCardNumber(
                  account.rib ?? ""
                )}`
              : "Make a Withdrawal"}
          </DialogTitle>
        ...
        {!waiting ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 space-y-4">
            <div className="flex space-x-2">
              <span className="w-3 h-3 bg-yellow-500 rounded-full animate-bounce delay-75"></span>
              <span className="w-3 h-3 bg-orange-500 rounded-full animate-bounce delay-150"></span>
              <span className="w-3 h-3 bg-yellow-500 rounded-full animate-bounce delay-300"></span>
            </div>
            <p className="text-center text-sm text-gray-600">
              Do not close this dialog. Waiting for withdrawal confirmation...
            </p>
          </div>
        )}
      </DialogContent>
